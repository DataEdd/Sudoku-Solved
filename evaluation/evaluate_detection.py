...
Runs ``app.core.extraction.detect_grid`` — the deterministic 4-step
fallback chain used by ``/api/extract`` — against the 38 newspaper
photos in ``Examples/Ground Example/`` and compares the detected outer
corners against the annotated 16-point ground truth.

Single-run only — ``detect_grid`` has no tunable knobs.
...
from app.core.extraction import detect_grid  # noqa: E402

GT_PATH = Path(__file__).parent / "ground_truth.json"
OUTPUT_PATH = Path(__file__).parent / "detection_results.json"
...
def print_report(per_image: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    print()
    print("=" * 96)
    print("detect_grid — production detector vs 38-image GT")
    print("=" * 96)
...
def main() -> None:
    ...
    payload = {
        "command": "python -m evaluation.evaluate_detection",
        "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "notes": (
            "Runs the production detect_grid fallback chain against the "
            "38-image GT set. Detector is deterministic, so a single pass "
            "is sufficient. Any non-None return from detect_grid counts as "
            "'detected'; IoU and per-corner Euclidean distance are computed "
            "against the 4 outer corners extracted from corners_16 at "
            "indices [0, 3, 15, 12] (TL, TR, BR, BL)."
        ),
        "summary": summary,
        "per_image": per_image,
    }

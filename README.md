...
| Digit OCR (custom CNN) | **66.6%** filled-cell · **96.9%** empty-cell · **80.6%** overall | `evaluation/evaluate_ocr.py` |
...
Trained on MNIST (handwritten digits) + ~4,500 system-font-rendered printed digits drawn from 67 allowlist-validated Latin-digit fonts (of 371 discovered in `/System/Library/Fonts/*` — the rest are rejected by a two-stage filter: font-family allowlist then a per-font distinct-Latin-digit rendering check that catches `LastResort.otf`, `Symbol.ttf`, dingbats, and CJK/math fallbacks that would otherwise ship silent label noise) with rotation/noise/blur augmentation + ~1,800 Chars74K held-out-font printed digits + 5,000 synthetic empty-cell variants. Best test-split accuracy **99.40%** at epoch 27; real-photo accuracy on the 38-image ground truth is **66.6% filled-cell / 96.9% empty-cell / 80.6% overall** via the production detect_grid path, rising to **84.7% / 98.5% / 90.8%** with ground-truth corners and the 8-point piecewise warp (the upper bound once `extract_cells_piecewise` lands in `/api/extract`).
...
- [x] **102K-parameter custom CNN** for per-cell digit recognition, trained on MNIST + 67 allowlist-validated Latin-digit system fonts + Chars74K held-out fonts + synthetic empty cells; 99.40% synthetic test accuracy (font-disjoint Chars74K split), 66.6% filled / 96.9% empty / 80.6% overall on real photos via detect_grid, 84.7% filled / 98.5% empty / 90.8% overall with ground-truth corners + piecewise warp (upper bound)
- [x] **Architecture ablation study** — 3×3×3 grid over `(depth, channel width, dropout)` under identical training protocol; results in `evaluation/ablation_results.json`, analysis in [`notebooks/03_ocr.ipynb`](notebooks/03_ocr.ipynb)
...
- [ ] Wire `extract_cells_piecewise` (`app/core/extraction.py`) into `/api/extract` so curved newspaper pages use the 8-point piecewise warp. Currently it's only used in `evaluate_ocr.py --piecewise` and in the annotation preview.
